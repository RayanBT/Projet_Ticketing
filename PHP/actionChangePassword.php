...
if (isset($_POST["changer_mot_de_passe"])) {
    ...
    $check_password_query = "SELECT login FROM $tab WHERE login = ? AND Mdp = MD5(?)";
    $check_password_stmt = mysqli_prepare($connection, $check_password_query);
    mysqli_stmt_bind_param($check_password_stmt, 'ss', $login, $mot_de_passe_actuel);
    ...
    if (mysqli_stmt_num_rows($check_password_stmt) > 0) {
        ...
        $update_password_query = "UPDATE $tab SET Mdp = MD5(?) WHERE login = ?";
        $update_password_stmt = mysqli_prepare($connection, $update_password_query);
        mysqli_stmt_bind_param($update_password_stmt, 'ss', $nouveau_mot_de_passe, $login);
